    ...
    httpRequest.onreadystatechange = function(){
        if(httpRequest.readyState === httpRequest.DONE) {
            if(httpRequest.status === 200) {
				...
				/* CANADA */
				/*data_len = labels.length*/
				data_len = labels.length - 1
				/* CANADA */
				/*var last_date = new Date(labels.at(-1))
				for(i=1; i<=5;i++){
					last_date.setDate(last_date.getDate() + 21)
					labels.push(last_date.toISOString().substring(0, 10))
				}*/
				let last_date = labels.at(-1)
				for(i=1;i<=4;i++){
					labels.push(last_date+i)
				}
				...
				for(const [key, time_series] of Object.entries(received_pred)){
					...
					/* CANADA */
					/*diff = processed_data[key].at(-1) - time_series[0][0]*/
					diff=0
					for(const value of time_series){
						processed_pred[key].push(value[0]+diff)
					}
				}
